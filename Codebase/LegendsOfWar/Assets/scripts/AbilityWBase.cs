...
public abstract class AbilityWBase : AbilityBase
{
	...
	{
		base.Update();
		if ( ( Input.GetKeyDown( KeyCode.W ) && !HeroCamScript.onHero ) || Input.GetKeyDown( KeyCode
			.Alpha2 ) || Input.GetKeyDown( KeyCode.Keypad2 ) )
			TryCast();
		ToggleCursor( ( Input.GetKey( KeyCode.W ) && !HeroCamScript.onHero ) || Input.GetKey(
...
public abstract class AbilityWBase : AbilityBase
{
	protected override void Update()
	{
		...
		if ( ( Input.GetKeyDown( KeyCode.W ) && !HeroCamScript.onHero ) ||
			...
			TryCast();
		ToggleCursor( ( Input.GetKey( KeyCode.W ) && !HeroCamScript.onHero ) ||
