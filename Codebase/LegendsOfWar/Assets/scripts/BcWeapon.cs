...
public class BcWeapon : MonoBehaviour
{
	...
	public void SpawnBullet()
	{
		for ( int i = 0; i < bulletsPerShot; ++i )
		{
			...
			rigidBody.velocity = transform.forward * ( bulletSpeed + bulletSpeedDelta * ( Random.value - 0.5f ) * 2.0f ) + transform.up * ( verticalSpread * ( Random.value - 0.5f ) * 2.0f ) + transform.right * ( horizontalSpread * ( Random.value - 0.5f ) * 2.0f );
			ConstantForce force = newBullet.AddComponent<ConstantForce>();
			force.force = ( bulletAcceleration.x * transform.forward + bulletAcceleration.y * transform.up + bulletAcceleration.z * transform.right ) * accelerationScale;
	...
	{
		Vector3 result = displayOffset.z * transform.forward * transform.lossyScale.z + displayOffset.y * transform.up * transform.lossyScale.y + displayOffset.x * transform.right * transform.lossyScale.x;
		if ( swap )
	...
	void DrawGizmos()
	{
		if ( this.enabled )
		{
			...
			for ( float i = 0.0f; i < clipSize / rateOfFire; i += 1.0f )
			{
				...
				Gizmos.DrawLine( DisplayPosition() - transform.up * ( height * 0 + 0 ) + transform.forward * i * displayScale, DisplayPosition() - transform.up * ( height * 0 - size ) + transform.forward * i * displayScale );
			}
			Gizmos.color = Color.white;
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( -height / 2 - height / 2 ) + transform.forward * 0, DisplayPosition() - transform.up * ( -height / 2 - height / 2 ) + transform.forward * width );
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( -height / 2 + height / 2 ) + transform.forward * 0, DisplayPosition() - transform.up * ( -height / 2 + height / 2 ) + transform.forward * width );
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( -height / 2 - height / 2 ) + transform.forward * width, DisplayPosition() - transform.up * ( -height / 2 + height / 2 ) + transform.forward * width );
			Gizmos.color = Color.green;
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( -height / 2 ) + transform.forward * 0, DisplayPosition() - transform.up * ( -height / 2 ) + transform.forward * Mathf.Max( 0, currentAmmo + shootTimer * rateOfFire ) / clipSize * width );
			for ( float i = 0; i < clipSize; i += 1 )
			{
				...
				Gizmos.DrawLine( DisplayPosition() - transform.up * ( +height * 1 + 0 ) + transform.forward * i * width / clipSize, DisplayPosition() - transform.up * ( +height * 1 - size ) + transform.forward * i * width / clipSize );
			}
			Gizmos.color = Color.white;
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( +height / 2 - height / 2 ) + transform.forward * 0, DisplayPosition() - transform.up * ( +height / 2 - height / 2 ) + transform.forward * width );
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( +height / 2 + height / 2 ) + transform.forward * 0, DisplayPosition() - transform.up * ( +height / 2 + height / 2 ) + transform.forward * width );
			...
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( +height / 2 ) + transform.forward * 0, DisplayPosition() - transform.up * ( +height / 2 ) + transform.forward * Mathf.Max( 0, currentAmmo ) / clipSize * width );
			...
			Gizmos.color = Color.white;
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( -height / 2 - height / 2 ) - transform.forward * 0, DisplayPosition() - transform.up * ( -height / 2 - height / 2 ) - transform.forward * reloadTime * displayScale );
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( -height / 2 + height / 2 ) - transform.forward * 0, DisplayPosition() - transform.up * ( -height / 2 + height / 2 ) - transform.forward * reloadTime * displayScale );
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( -height / 2 - height / 2 ) - transform.forward * reloadTime * displayScale, DisplayPosition() - transform.up * ( -height / 2 + height / 2 ) - transform.forward * reloadTime * displayScale );
			Gizmos.color = Color.red;
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( -height / 2 ) - transform.forward * 0, DisplayPosition() - transform.up * ( -height / 2 ) - transform.forward * reloadTimer * displayScale );
			Gizmos.color = Color.white;
			Gizmos.DrawLine( transform.position, transform.position + transform.up * ( height * 2 ) * IsOverCannon() );
			Gizmos.DrawLine( transform.position + transform.up * ( height * 2 ) * IsOverCannon(), DisplayPosition() - transform.up * ( height * 2 ) * IsOverCannon() );
			Gizmos.DrawLine( DisplayPosition() - transform.up * ( height * 2 ) * IsOverCannon(), DisplayPosition() );
			if ( autofire )
			...
			for ( int i = 0; i < bulletsPerShot; i++ )
			{
				...
				if ( i % 10 == 0 )
					size = 2.0f;
				Gizmos.DrawLine( transform.position + transform.up * i / bulletsPerShot * displayScale * 2, transform.position + transform.forward * size * displayScale + transform.up * i / bulletsPerShot * displayScale * 2 );
			}
	...
	void OnSceneGUI()
	{
		...
		if ( tW.enabled )
		{
			...
			if ( showDisplayHandles.boolValue )
			{
				...
				displayScale.floatValue = Mathf.Max( 0.1f, Handles.ScaleSlider( displayScale.floatValue, offset + tW.DisplayOffset(), -tW.transform.up, tW.transform.rotation, 0.75f * HandleUtility.GetHandleSize( offset + tW.DisplayOffset() ), 0 ) );
			}
